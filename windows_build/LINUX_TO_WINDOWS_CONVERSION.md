...
| `atak_osmdroid_sqlite_footprint.py` | No — SQLite builder lazy-imports for ATAK_metadata footprint |
...
**Status:** ✅ in launcher files (`freeze_support` on **both** launchers); **not** generated by sync
...
| Step | What |
|------|------|
| B0 | `scripts/windows_bundle_manifest.py` — canonical hidden imports, pip deps, script bundles |
| B0b | `scripts/audit_windows_bundle.py` — py_compile + optional import smoke test (runs before PyInstaller) |
| B1 | Bundle `windows_build/data/` → `scripts\data` in PyInstaller |
| B2 | Bundle Tcl/Tk 8.6 from Python install |
| B3 | Hidden-import all modules from manifest (includes `atak_osmdroid_sqlite_footprint`, multiprocessing spawn) |
| B4 | `--collect-all certifi requests mgrs` (SSL CA bundle + HTTP stack + MGRS) |
